"""
Parsing des logs ACE EVO en temps réel → état de session courant
(pilotes connectés, tours, secteurs, leaderboard).

Extrait de app/routes/live.py pour séparer la logique métier (parsing/état)
de la couche route (HTTP/SSE).
"""
import logging
import os
import re
...
def fmt_ms(ms: int | None) -> str | None:
    if not ms:
        return None
    m   = ms // 60000
    s   = (ms % 60000) // 1000
...
def fmt_sector(ms: int | None) -> str | None:
    if not ms:
        return None
    s  = ms // 1000
    cs = ms % 1000
...
def iter_log_lines(since_hours: int = 12, server_id: int = 1):
...
def build_state_cached(server_id: int = 1) -> dict:
    """Returns a cached build_state() result, re-parsed at most every _STATE_TTL seconds."""
    now = time.monotonic()
    with _state_cache_lock:
        cached = _state_cache.get(server_id)
        if cached and now - cached[0] < _STATE_TTL:
            return cached[1]
    state = build_state(server_id)
    with _state_cache_lock:
        _state_cache[server_id] = (now, state)
    return state


def build_state(server_id: int = 1) -> dict:
    ...
    def _car_laps(car_id: str):
        ...
        for ev in car_timeline.get(car_id, []):
            if ev["type"] == "sector":
                ...
                laps.append({
                    "lap_num": len(laps) + 1,
                    "lap_ms":  lms, "lap_str": fmt_ms(lms),
                    "s1_ms":   s1,  "s1_str":  fmt_sector(s1),
                    "s2_ms":   s2,  "s2_str":  fmt_sector(s2),
                    "s3_ms":   s3,  "s3_str":  fmt_sector(s3),
                })
                pending = []
    ...
    for _steam_sid, d in drivers.items():
        ...
        leaderboard.append({
            **d,
            ...
            "lap_count":    len(laps),
            "best_lap_ms":  best_ms,
            "best_lap_str": fmt_ms(best_ms),
            "last_lap_ms":  last_ms,
            "last_lap_str": fmt_ms(last_ms),
            # Personal bests secteur (pour coloration violet/vert)
            "pb_s1_ms": pb_s1_ms, "pb_s1_str": fmt_sector(pb_s1_ms),
            "pb_s2_ms": pb_s2_ms, "pb_s2_str": fmt_sector(pb_s2_ms),
            "pb_s3_ms": pb_s3_ms, "pb_s3_str": fmt_sector(pb_s3_ms),
            # Secteurs du tour en cours
            "curr_s1_ms": cs1, "curr_s1_str": fmt_sector(cs1),
            "curr_s2_ms": cs2, "curr_s2_str": fmt_sector(cs2),
        })
    ...
    for entry in leaderboard:
        if leader_ms and entry["best_lap_ms"]:
            ...
            entry["gap_str"] = f"+{fmt_ms(gap)}" if gap > 0 else "—"
        else:
            entry["gap_str"] = None
